...
> Brand-new pages carry a `new_page` block, then RouteBundleSynth freezes a bundle, then
> `merge-routes` folds it into the index (additive, idempotent).
...
> - **Done (new_page):** Basic-Auth 401 (23), the beelzebub fake-secrets bundle (21, covering
...
_Extracted from TANNER/SNARE/Glastopf, product/CVE honeypots (Struts, Tomcat, Citrix, Drupal, phpMyAdmin, OWA, and others), LLM/config honeypots (galah, beelzebub, Lophiid, modpot), and a broad sweep. All items are safely canned (output-only or bounded/computed reflection). They never execute attacker input._
...
| 6 | **XXE to file disclosure** | TANNER, Reactive-WAH (paper) | POST XML w/ `<!DOCTYPE…ENTITY SYSTEM "file:///etc/passwd">` | reuse existing canned `/etc/passwd`, bounded-reflected in parse output | attack-Signature | S-M | Med-High |
...
| 12 | **SQLi result rows (UNION/dump)** | TANNER | `UNION SELECT`, `information_schema` | fake users/version rows, extends the current error-only SQLi | product-deep / extend SQLi sig | M-H | Med-High |
...
| 18 | **phpMyAdmin login + fake authed panel** | Glastopf, modpot, phpmyadmin_honeypot, Lophiid | `/phpmyadmin`, `/pma`, `login.php` | static PMA login, then fake authed UI + fake phpinfo | product-deep | M | Med |
| 19 | **Exchange/OWA fingerprint + logon** | owa-honeypot, modpot | `/owa`,`/ecp`,`/EWS`,`/Autodiscover` and similar | 401 `Server: Microsoft-IIS/7.5` + `X-Powered-By: ASP.NET`; fake `logon.aspx` (15.1.1466) | product-deep | M | Med |
...
| 25 | **WordPress endpoint pack** | wordpot, HoneyPress | `recent-backups`/`downloadfile`; `/?author=N`; `/readme.html`; wp-mobile-detector `resize.php`; `*thumb`/`uploadify` | recent-backups reuses `/etc/passwd`; author-enum returns username `admin`; readme returns `Version 2.8`; timthumb returns `no image specified` | endpoint-Emulators (batch) | S each | Med |
...
| 28 | **Lophiid edge-appliance packs** (Ivanti, Fortinet, PAN GlobalProtect, SonicWall, F5 Big-IP, ColdFusion, TeamCity, Metabase, CraftCMS, GeoServer, OFBiz, Cleo, Aspera, and more) | Lophiid | product-specific RECON + CVE URIs | captured real HTML/JSON + per-product `Server`/version header | product-deep (bulk) | M-H per product | High **(licensing flag)** |
...
5. **Confluence CVE-2022-26134 (`${` to `X-Cmd-Response: uid=0(root)`).** Single canned response header satisfies the exploit check; one of the most-scanned RCEs.
6. **XXE to canned `/etc/passwd` Signature.** Near-free: reuses funnypot's existing passwd body inside an XML-parse response.
...
- **honeyup zip-bomb `public_html.zip`** (the harmful decompression bomb): retaliatory/harm-causing; abuse and legal risk; against reflect-never-harm. NOT built. A SAFE substitute IS built for the demo honeypot instead: a *bounded nested decoy* archive (scripts/build-decoys.sh builds demo/decoys/backup.{zip,tar.gz}), served for `.zip`/`.tar.gz` probes that would 404. Fixed depth, each layer a normal archive of the next down to fabricated secrets. It wastes an attacker's *time* re-extracting, never their RAM/disk (fully extracts to a few KB). Off-switch `FUNNYPOT_DECOY_ARCHIVE=0`.
