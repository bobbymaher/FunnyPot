...
Grounded in the shipped 5.06 MB `nuclei-index.full.php` (upstream `2ec9141`). The cap's headline
payoff is **anti-detection / believability**, NOT size: `GET /` is only ~14% of the file and the
mega-5 paths ~18%; the other ~82% is legitimate breadth across 3,600+ real paths + detect metadata
and must NOT be cut. Realistic size after cap ≈ **4.5 MB** (keep full detect).

## Where the mass is (measured)
- Multi-bundle keys: **202** (3,587 are singletons). Keys with >20 bundles: **5 only** —
  `GET /` 1252, `/index.php` 171, `/login` 115, `/wp-admin/admin-ajax.php` 27, `/login.html` 21.
