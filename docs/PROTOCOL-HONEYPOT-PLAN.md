# Protocol Honeypot Plan — multi-protocol (L4/TCP) engine alongside the HTTP engine

Status: design + build plan. Not built.

funnypot today is an HTTP honeypot: nginx/php-fpm answers scanner probes with
fake-vulnerable responses synthesized from data-driven templates. This document
plans a second engine that listens on raw TCP and speaks **wire protocols**
(redis, telnet, ftp, smtp, memcached, and — the hard one — ssh), reusing
funnypot's "everything is a bounded template" philosophy and its
...
logged — while the honeypot never executes attacker input and never becomes a
real service.**
...
Read before proposing anything new — the protocol engine must feel like the same
project.
...
- **Bounded dynamic vocabulary.** `DirectiveRenderer` fills `{{...}}` markers
  ...
  attacker bytes), `{{pick:a,b,c}}`, `{{canary.KEY}}`. Unknown directive → build
  failure. Reflected values are inserted once and never re-scanned, so an
  attacker-supplied `{{...}}` stays inert literal text. **This is the safety
  core — reuse it verbatim.**
- **Compile-time safety lint.** The compiler rejects unknown directives, CR/LF
  ...
  that only survive by reflection. The protocol compiler needs the analogous
...
The protocol engine is a **new listener front-end + a wire-template format +
a shared shell layer**, bolted onto that same spine (compiled artifact →
interpreter → Observer → hit log → dashboard).
...
|---|---|---|---|---|
| **openswoole** | PECL C extension (large build) | coroutine per connection, true async, very high concurrency | must compile/install the extension into the image; changes the whole runtime model | Powerful but exactly the heavyweight dep Bob wants to avoid. **Reject.** |
...
**A generic zero-dependency listener binary — `stream_socket_server` +
...
- One `stream_select` loop multiplexes the listen socket + all live connections.
...
- **Keeps the runtime PHP-only** — the same invariant that lets the HTTP runtime
  ship without composer. The listener is `php` + the standard `sockets`/stream
  functions, nothing to `composer require` or compile.
...
max connections per source IP, max inbound buffer per connection (drop + close
...
list of request→response rules. Plaintext protocols become **pure data**; binary
...
The redis template above is **entirely data** — no PHP written per-protocol. The
...
request→response bytes for SSH** the way you can for redis or telnet — there are
...
- There is **no maintained pure-PHP SSH *server* library.** Hand-rolling one
  ...
  PHP — weeks of security-sensitive crypto code, exactly the kind of thing that
  becomes a real vulnerability if you get it wrong. **Reject.**
- For comparison, the reference honeypots don't hand-roll it either: **Cowrie**
  ...
  *shell above it*; **OpenCanary's "SSH"** doesn't complete a handshake at all —
  it presents a version banner and logs connection/auth attempts.

That comparison gives a clean **tiered** answer:

### Tier 1 — SSH banner + connection logging (pure PHP, in-model, ship first)
...
- Read the client's banner + first `KEXINIT` bytes, log them as intel (client
...
This catches banner-grabbers and connection loggers — meaningful coverage — with
essentially no cost.
...
SSH crypto using a battle-tested library, does accept-all auth, and forwards the
...
- Python + Paramiko/AsyncSSH is an equivalent option if a Python runtime is
  preferred.
- **The sidecar owns transport only. It contains zero emulation logic.** It does
  ...
  sidecar and PHP is line-oriented: `{session-id}\t{command}` → `{output-bytes}`.
...
1. Ship **Tier 1 (banner + logging)** now — pure PHP, in the template model.
...
**Need a codec** (binary / length-prefixed / stateful framing — a small shipped
PHP class in addition to the template):
...
- **SSH, TLS** — full crypto, not a codec problem. SSH → sidecar (§3). **TLS is
  out**: nginx already terminates TLS for the HTTP engine; we do not build a
  second TLS stack.
...
1. **Inbound framing** — consume the connection's byte buffer and yield complete
   ...
   3-byte length + seq header, then that many payload bytes). Partial data stays
...
lint-covered, and — like everything else — never execute input, only frame it.
...
- **Fake filesystem** — a static, seeded, read-only tree loaded from a data file
  ...
  built from the same directives (`/etc/passwd` → `{{canned.passwd}}`,
  `~/.aws/credentials` → seeded `{{fake.*}}` blob). Cowrie-style content, but
  **plain data, not pickled code** — nothing in the tree is ever deserialized
  into executable state.
- **Command table** — maps a command (+ optional arg pattern) to canned output:
  `whoami`→`root`, `id`→fake uid line, `uname -a`→seeded kernel string,
  `ls`/`ls -la`→listing derived from the fake fs, `cat <path>`→fake file content,
  `ps`/`netstat`→canned tables. Unknown command → `bash: <cmd>: command not
  found`. **`wget`/`curl`/`nc`/`pip install` are emulated as canned text — the
  requested URL is logged as intel and NEVER dereferenced.**
...
`eval`, **no** real filesystem access, **no** outbound socket from the handler.
The fake `cat`, fake `ls`, and fake `wget` are string lookups. The DirectiveRenderer
guarantees any reflected attacker bytes (e.g. an echoed filename) stay inert.

---

## 6. Packaging in the demo docker + safety invariants

### Packaging

The demo container already runs php-fpm + nginx via `demo/entrypoint.sh`. Add:
...
- **Listener processes** — one `php bin/funnypot-listen <proto> <port>` per
  ...
  each and `wait`s + restarts). Process-per-protocol gives isolation and
  independent restart.
- **Ports** — publish the protocol ports in `demo/docker-compose.yml` alongside
  ...
  the sidecar** (avoid `22` — it collides with the host's real sshd; map
...
- **Emulate output, never execute input.** Command table + template lookups only.
  No `eval`/`shell_exec`/`proc_open`/`system`, no real fs, no outbound fetch —
  in the listeners, the shell layer, and every codec. Reuse the closed
  `DirectiveRenderer` vocabulary so reflected attacker bytes stay inert literal
  text (insert-once, never re-scanned).
- **Reflect-never-harm.** No decompression/RESP/amplification bombs; every
  ...
  per-session output caps; idle + total timeouts; caps on concurrent connections
  per listener and per source IP; inbound buffer cap (drop + close on overflow)
...
- **Inert by default.** Like the HTTP `Config`, default to detect/log only
  (e.g. SSH Tier-1 banner + auth logging, connection logging) and require an
...
- **Phase 0 — listener runtime + compiler.** Generic `bin/funnypot-listen`
  (select loop, connection/buffer/timeout caps, protocol dispatch); the
  `templates/protocol/*.yaml` → `resources/compiled/funnypot-protocols.php`
...
- **Phase 1 — plaintext protocols as pure data.** redis (with the shared `resp`
  ...
  template, zero new PHP." Add SSH **Tier-1 banner + logging** here (it's just a
  `banner:` template).
...
- **Phase 4 — binary codec protocols.** MySQL handshake/auth (codec + template),
  optionally MSSQL. TLS/RDP/SMB explicitly out.
- **Phase 5 — packaging + dashboard.** Process supervisor, docker ports,
...
- **SSH transport is the genuine hard part.** No PHP server lib exists; hand-
  rolling crypto is rejected. The plan's answer (banner-only in PHP + a sidecar
...
- **Select-loop correctness.** Partial reads, per-connection state machines,
  backpressure, and — critically — **never blocking the whole loop on one slow or
...
- **Anti-fingerprinting has a ceiling.** Honeypots are detectable via tiny
  ...
  and the seeded per-attacker fakes + latency jitter (already in `Config`) help,
...
2. **The SSH sidecar (Go/Python) — in or out?** It's the only non-PHP piece.
   ...
   (c) defer the decision — Phases 0–2 don't need it.
3. **How far into binary protocols?** MySQL is doable but fiddly and
   fingerprintable. Worth Phase 4, or stop at plaintext + shell?
