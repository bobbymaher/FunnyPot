...
> either **reports the match** (detect mode) or **synthesizes a response that satisfies those
> templates' matchers** (respond mode) — so a scanner walks away with a fat, *coherent* vuln
> report, wasting the attacker's time.

Package: `funnypot/funnypot` · namespace `Funnypot\` · PHP `>=8.0`
...
- Non-invertible subsets (corrected counts): `raw:` = **3,438** (list-item form; not 12),
  `payloads:` 686, `body:` 155, interactsh/OOB **~528** (unfakeable — callback goes to the
  scanner's own collaborator), xpath 38.
- Modifiers to honor exactly: `matchers-condition: and` in **7,467** templates (one OUT matcher
  ⇒ whole template OUT), `negative: true` in **229**, `dsl` in 3,463, `compare_versions` in
  **381** files (302 non-raw, many single-step and *readily* invertible).
- **`MatchStatusCode` is OR-only, exact** ("Status codes don't support AND conditions",
  `match.go`) → **one response has exactly one status line.** This is the physical constraint
  that reshapes the whole design (see §5).
- Post-Gate-A invertible pool ≈ **6,400 templates → ~4,700 `(method,path)` groups**. The final
  IN count is only known after per-witness validation (§6); ship with an auditable `skipped.json`.
...
`RequestContext`; a single hash probe on `"METHOD normalized-path"` either misses (→ `null`,
app serves its own 404) or hits and returns a `Detection` (**detect**) or a synthesized
...
(~0.5 MB / ~4,700 keys). Per-request cost = one COW hash probe; miss = same probe → `null`
(honeypot traffic is miss-heavy). **No objects/closures/`::class` in the file** — recipe
...
**Entry schema** (`schema=1`): every key → a **list of bundles** (singletons have one; collision
heads have several):
...
sha256), `skipped.json` (`id → reason`, coverage audit), `UPSTREAM-LICENSE.md`.
...
`and` → template IN only if **every** matcher invertible; `or`/default → IN if **≥1** invertible
(satisfy the cheapest, ignore the rest).

| Matcher / modifier | Verdict | Satisfaction |
|---|---|---|
| `status` | EASY | emit one code from the OR-set |
| `size` | EASY | pad matched part to `len==N`; **exact-size ⇒ bundle-exclusive** (A4) |
...
| **`regex` anchored `^…$`/`$`** | MED+shape | **whole-body-exclusive** constraint, NOT a free word (A1) — `compatible()` rule 5 |
| `dsl` — whitelist | SUBSET | `status_code==N`, `contains[/tolower]/(body|all_headers,'x')`, `!contains`→forbidden, `contains_all/any`, `len(x) ==/>/< N`, `startswith/endswith`, `&&`/`||` |
...
- **A2 — dynamic `{{…}}` literals:** screen every word/regex/dsl literal for `{{…}}`.
  Runtime-resolvable (`{{Hostname}}`, `{{Host}}`) → synthesizer supports; everything else
  (`randstr`, `md5`, `{{interactsh…}}`, extracted `{{result}}`) ⇒ matcher OUT → fold. 302 files
  affected; without this they compile IN and are **dead on the wire** under `and`.
- **A1 — anchor kind:** tag regex anchors; `$` or `^…$` ⇒ whole-body-exclusive (rule 5).
- **B6 — intra-template satisfiability:** after building a plan verify `status∩≠∅`,
  `required∩forbidden=∅`, size consistent; contradiction ⇒ fold OUT with a reason.
...
| `gate fn(RequestContext):bool` | `fn=>false` | app suspicion predicate; false ⇒ respond null |
...
released tag** → compile → golden test → atomic write (tmp/fsync/rename/opcache-invalidate) →
...
*everything at once*" is impossible wherever templates disagree on status/size/product — and
forcing it (two `Server:` headers, impossible status) is itself the honeypot tell. Deliverable
...
conflict) co-bundle and one `/` response reports WordPress **and** Jenkins **and** phpMyAdmin
simultaneously — unmasked on the first multi-template scan. `coherent` = **one product persona**.
...
checked to never introduce a forbidden substring or a CRLF/NUL (C8). Stateless → consistent
...
200) is a tell independent of persona. Honest value prop: **cost imposition on mass low-effort
...
- **Root-path (`sig=1`)**: `GET /` fires only with a probe signature present, never for ordinary
  homepage traffic.
- **`trustedBypass` checked before everything** so the org's own ASM/nuclei sees real posture —
...
- **Two nuclei lanes:** pinned digest (reproducible CI) + floating-latest (allowed to fail,
  alerts on nuclei-semantic drift — fidelity target is the nuclei the *attacker* runs).
...
Prove compile→normalize→hash-probe→`Detection` end-to-end on ~5 dead-simple **singleton**
templates that are pure `status + word(part:body)` — no regex, no negatives, no encoded paths,
...
Proof: per-bundle coverage asserted; two seeds → two self-consistent personas; no response
...
5. **Client-IP persona tell** — accept documented residual (two-IP correlation defeats persona),
   value prop = mass-scanner cost imposition, not analyst-proof?
